...
st.caption("Análise matemática de alta precisão baseada em Odds e Probabilidade Implícita.")
...
def calcular_probabilidades_odds(fixture_id, home_team, away_team):
    ...
    # 1. Tentativa por ODDS em tempo real
        ...
        if res_odds.status_code == 200:
            ...
            if odds_data:
                ...
                if bookmakers:
                    ...
                    for bet in bets:
                        if bet.get("id") == 1:  # Mercado 1X2 (Match Winner)
                            values = bet.get("values", [])
                            odd_home = float(next((item["odd"] for item in values if item["value"] == "Home"), 0))
                            odd_draw = float(next((item["odd"] for item in values if item["value"] == "Draw"), 0))
                            odd_away = float(next((item["odd"] for item in values if item["value"] == "Away"), 0))

                            if odd_home > 0 and odd_draw > 0 and odd_away > 0:
                                # Conversão da Probabilidade Implícita: (1 / Odd)
                                prob_h_raw = 1 / odd_home
                                prob_d_raw = 1 / odd_draw
                                prob_a_raw = 1 / odd_away
                                total_raw = prob_h_raw + prob_d_raw + prob_a_raw  # Remove a margem da casa (Vigorish)
                                ...
                                return f"{p_home}%", f"{p_draw}%", f"{p_away}%", advice, f"Odds de Mercado (@{odd_home} / @{odd_draw} / @{odd_away})"
    except Exception:
        pass

    # 2. Algoritmo de Contingência Baseado na API (Predictions / Desempenho)
    ...
    try:
        ...
        if res_pred.status_code == 200:
            ...
            if pred_data:
                ...
                form_home = int(comparison.get("form", {}).get("home", "50%").replace("%", ""))
                form_away = int(comparison.get("form", {}).get("away", "50%").replace("%", ""))
                
                score_home = form_home + 15 # Vantagem de jogar em casa
                ...
                return f"{p_home}%", f"{p_draw}%", f"{p_away}%", advice, "Histórico & Forma Recente"
    except Exception:
        pass

    # 3. Fallback estatístico padrão para partidas sem registros na API
    return "45%", "30%", "25%", f"Dupla Chance ({home_team} ou Empate)", "Estimativa Padrão de Mando"
