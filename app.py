...
if st.button("Buscar Oportunidades"):
    ...
    url = f"https://v3.football.api-sports.io/fixtures?league={liga_id}&season=2026&date={data_str}"
    ...
    if response.status_code == 200:
        ...
        if not dados:
            # Busca alternativa: tenta pegar os próximos 10 jogos independente da data
            url_next = f"https://v3.football.api-sports.io/fixtures?league={liga_id}&next=10"
            dados = requests.get(url_next, headers=HEADERS).json().get(
                "response", []
            )
        ...
        if not dados:
            ...
            for jogo in dados:
                ...
                st.write(f"⏰ **Horário:** {horario} | **Status:** {status}")
